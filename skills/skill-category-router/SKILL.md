...
The preferred human entry point is `/skill-master`, defined in `commands/skill-master.md`. That command routes natural language through `src/routing/skill-master-routing.json` before loading the selected advisor skill.
...
3. Read `src/routing/skill-master-routing.json` when available.
